...
import { LineChart, DonutChart } from 'patternfly-react';
...
class ServiceMetrics extends React.Component<ServiceId, ServiceMetricsState> {
  ...
  health() {
    ...
    if (this.state.health) {
      // const healthy = this.round(100 * this.state.health.healthyReplicas / this.state.health.totalReplicas);
  ...
  renderHealth() {
    ...
    if (health) {
      console.log('Rendering health donut');
      return (
        <DonutChart
          id={'health-donut'}
    ...
    console.log('Rendering no health donut');
    // const data = {
    //   colors: { 'N/A': '#707070' },
    //   columns: [['N/A', 1]],
    //   type: 'donut'
  ...
  renderChart(id: string, title: string, columns: [string, number][]) {
    ...
    return <LineChart id={id} title={title} data={data} axis={axis} point={{ show: false }} />;
  }
